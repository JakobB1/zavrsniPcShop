...
public class Start {
	...
	private Korisnik korisnikPostaviVrijednosti(Korisnik k) {
		k.setSifra(Ulaz.ucitajInt("Unesite sifru: ", "Sifra mora biti cijeli broj", 1, Integer.MAX_VALUE));
		k.setIme(Ulaz.ucitajString("Unesite ime korisnika: ", "Ime obavezno"));
		k.setPrezime(Ulaz.ucitajString("Unesi prezime korisnika: ", "Prezime obavezno"));
		k.setEmail(Ulaz.ucitajString("Unesi email korisnika: ", "Email obavezan"));
	...
	private void korisnikStravke(String naslov) {
		...
		if (korisnici.size() == 0) {
			...
			for (int i = 0; i < korisnici.size(); i++) {
				k = korisnici.get(i);
				System.out.println((i + 1) + ". " + k.getIme() + " " + k.getPrezime() + " " + k.getEmail() + " "
	...
	private Djelatnik djelatnikPostaviVrijednosti(Djelatnik d) {
		d.setSifra(Ulaz.ucitajInt("Unesite sifru: ", "Sifra mora biti cijeli broj", 1, Integer.MAX_VALUE));
		d.setIme(Ulaz.ucitajString("Unesi ime predavaca: ", "Ime obavezno"));
		d.setPrezime(Ulaz.ucitajString("Unesi prezime predavaca: ", "Prezime obavezno"));
	...
	private void djelatnikStavke(String naslov) {
		...
		if (djelatnici.size() == 0) {
			...
			for (int i = 0; i < djelatnici.size(); i++) {
				d = djelatnici.get(i);
				System.out.println((i + 1) + ". " + d.getIme() + " " + d.getPrezime() + " " + d.getPcshop());
			}
		}
	}
	...
	private Racun racunPostaviVrijednosti(Racun r) {
		r.setSifra(Ulaz.ucitajInt("Unesite sifru: ", "Sifra mora biti cijeli broj", 1, Integer.MAX_VALUE));
		r.setBrojracuna(Ulaz.ucitajInt("Unesi broj racuna :", "Niste unijeli cijeli broj", 1, 10000));
		r.setNazivproizvoda(Ulaz.ucitajString("Unesite naziv proizvoda: ", "Naziv proizvoda obavezan"));
		r.setCijena(Ulaz.ucitajDouble("Unesi cijenu: ", "Cijena mora biti decimalni broj", 0, 100000));
		...
		r.setNacinplacanja(Ulaz.ucitajString("Unesite nacin placanja: ", "Nacin placanja obavezan"));
	...
	private void racunStavke(String naslov) {
		...
		if (racuni.size() == 0) {
			...
			for (int i = 0; i < racuni.size(); i++) {
				r = racuni.get(i);
				System.out.println((i + 1) + ". " + r.getBrojracuna() + " " + r.getNazivproizvoda() + " "
						+ r.getCijena() + " " + r.getKolicina() + " " + r.getDatumpocetka() + " " + r.getDatumpocetka()
						+ " " + r.getNacinplacanja() + " " + r.getIznosracuna());
			}
		}
	}
	...
	private Proizvod proizvodPostaviVrijednosti(Proizvod p) {
		p.setSifra(Ulaz.ucitajInt("Unesite sifru: ", "Sifra mora biti cijeli broj", 1, Integer.MAX_VALUE));
		p.setNaziv(Ulaz.ucitajString("Unesite naziv :", "Naziv obavezan"));
		p.setCijena(Ulaz.ucitajDouble("Unesi cijenu: ", "Cijena mora biti decimalni broj", 0, 100000));
		...
		p.setDostupnostZaKupnju(
				Ulaz.ucitajBoolean("Unesite oznaku da li je " + "proizvod dostupan: ", "Kriva vrijednost"));
		p.setCertifikat(Ulaz.ucitajBoolean("Unesite oznaku da li je " + "proizvod certificiran: ", "Kriva vrijednost"));
	...
	private void proizvodStavke(String naslov) {
		...
		if (proizvodi.size() == 0) {
			...
			for (int i = 0; i < proizvodi.size(); i++) {
				p = proizvodi.get(i);
				System.out.println((i + 1) + ". " + p.getNaziv() + " " + p.getCijena() + " " + p.getOpisProizvoda()
						+ " " + p.getKodArtikla() + " " + p.isDostupnostZaKupnju() + " " + p.isCertifikat() + " "
						+ p.getGarancija());
			}
		}
	}
